...
| 资源 | 链接 |
|------|------|
| 演示视频（30–60 秒） | [待补充：B 站 / 网盘链接] |
| Windows 可执行包 | [待补充：Release 下载链接] |
| 源码仓库 | [待补充：GitHub 链接] |

> 建议在简历中附上**演示视频链接**，HR 通常不会 clone 工程自行运行。
...
本项目为**自学 Unity 期间独立完成**的游戏原型，目标岗位：Unity 客户端开发（实习 / 初级）。
...
|------|------|
| 开发者 | [你的名字] |
| 开发周期 | [例：2024.10 – 2025.06，约 X 个月] |
| 开发方式 | 独立开发（策划 / 程序 / 关卡 / 系统均为本人完成） |
| 美术资源 | 使用免费素材包（dungeon-pack-free 等），程序与系统设计为原创 |
...
- [ ] 减少 `FindObjectOfType` / `FindGameObjectWithTag` 的使用，改为 Inspector 引用或依赖注入
- [ ] 统一输入方案（背包 / 菜单迁移到 Input System）
- [ ] 子弹与掉落物引入对象池，降低 GC 压力
- [ ] 补充单元测试（背包堆叠、地牢连通性等）
- [ ] Boss 战设计深化（多阶段、技能预警）
- [ ] 添加 README 演示 GIF / 截图
...
| 地牢生成 | 战斗场景 | 背包系统 |
|:--------:|:--------:|:--------:|
| ![地牢](docs/screenshots/dungeon.png) | ![战斗](docs/screenshots/combat.png) | ![背包](docs/screenshots/inventory.png) |

> 建议截取 3–5 张图放入 `docs/screenshots/`，展示地牢全貌、战斗瞬间、背包 UI、升级 HUD。
